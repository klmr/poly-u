...
reads_all       <- reads[,c("cell","treatment","sample", "short")]
reads_all       <- summarise(group_by(reads_all, cell, treatment, sample, short), all = n())
reads           <- summarise(group_by(reads, cell, treatment, sample, short, mod), mod_count = n())
reads_summary   <- full_join(reads_all, reads)
rm(reads_all, reads)

reads_summary   <- mutate(reads_summary, frequency = 100*mod_count/all)
reads_summary   <- reads_summary[,c("cell", "treatment", "sample", "short", "mod", "frequency")]
reads_stats     <- summarise(group_by(reads_summary, cell, treatment, short, mod), mean = mean(frequency), sd = sd(frequency), frequencies=list(frequency))

#fold change
{
  reads_stats_f   <- reads_stats
  reads_stats_f   <- reads_stats_f[,c("cell", "treatment", "short", "mod", "mean")]
  ...
  reads_stats_f <- reads_stats_f[, c("cell", "short", "mod", "change")]
...
#t-test
{
  ...
  reads_stats_t   <- reads_stats_t[,c("cell", "treatment", "short", "mod", "frequencies")]
  ...
  rm(vector)
  reads_stats_t <- reads_stats_t[, c("cell", "short", "mod", "pval", "star")]
  
  reads_stats   <- full_join(reads_stats, reads_stats_t)
  reads_stats   <- reads_stats[,!(names(reads_stats) %in% c("frequencies"))]
}

reads_summary <- full_join(reads_summary, reads_stats, by= c("cell", "treatment", "mod", "short"))

write.csv(reads_summary, file = "../../plots/plot_3/data/freq_reads_points_by_mod_and_cell_treat_length_p_change.csv")
