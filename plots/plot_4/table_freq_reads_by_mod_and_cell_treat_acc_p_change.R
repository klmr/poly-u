...
reads           <- reads_filt_mrna[, c("mod","cell","treatment","sample", "pachy")]
rm(reads_filt_mrna_sample)

reads_all       <- reads[,c("cell","treatment","sample", "pachy")]
reads_all       <- summarise(group_by(reads_all, cell, treatment, sample, pachy), all = n())
reads           <- summarise(group_by(reads, cell, treatment, sample, pachy, mod), mod_count = n())
reads_summary   <- full_join(reads_all, reads)
rm(reads_all, reads)

reads_summary   <- mutate(reads_summary, frequency = 100*mod_count/all)
reads_summary   <- reads_summary[,c("cell", "treatment", "sample", "pachy", "mod", "frequency")]
reads_stats     <- summarise(group_by(reads_summary, cell, treatment, pachy, mod), mean = mean(frequency), sd = sd(frequency), frequencies=list(frequency))

#fold change
{
  reads_stats_f   <- reads_stats
  reads_stats_f   <- reads_stats_f[,c("cell", "treatment", "pachy", "mod", "mean")]
  ...
  reads_stats_f <- reads_stats_f[, c("cell", "pachy", "mod", "change")]
...
#t-test
{
  ...
  reads_stats_t   <- reads_stats_t[,c("cell", "treatment", "pachy", "mod", "frequencies")]
  ...
  rm(vector)
  reads_stats_t <- reads_stats_t[, c("cell", "pachy", "mod", "pval", "star")]
  
  reads_stats   <- full_join(reads_stats, reads_stats_t)
  reads_stats   <- reads_stats[,!(names(reads_stats) %in% c("frequencies"))]
}

reads_summary <- full_join(reads_summary, reads_stats, by = c("cell","treatment","pachy", "mod"))

write.csv(reads_summary, file = "../data/freq_reads_points_by_mod_and_cell_treat_acc_p_change_x_.csv")
